...
/**
 * @author Yash Sheth MCGCS port 3020 MCGSS port 3021
 */
public class MCGServer implements LibraryInterface {
	...
	static String serverNickName = "MCG";
	private static int portCS = 3020;
	private static int portSS = 3021;
	private static HostEnum host = HostEnum.HOST_3;
	static String fileContent = "";
	static MCGServer mcgserver = new MCGServer();
	...
	public static void main(String args[]) throws Exception {
		...
		try (DatagramSocket aSocket = new DatagramSocket();) {
			...
			System.out.println("Registering MCGSS details with central repo.");
			...
			if (packet.getData().toString().equals("TRUE"))
				System.out.println("MCGSS connection details are registered.");

			System.out.println("Registering MCGCS details with central repo.");
			...
			aSocket.receive(packet);
			if (packet.getData().toString().equals("TRUE"))
				System.out.println("MCGCS connection details are registered.");
		...
		System.out.println("McGill server is up.");
	}
	...
	private void activateServer() throws AlreadyBoundException, IOException {
		...
		try {
			...
			System.out.println("McGill UDP Socket 3030 Initiated for inter library communication. MCGCS");
			while (true) {
	...
	private void updateSequence(){
		expectedSequence++;
		System.out.println("Concordia : "+contactOtherServer("updateSequence", 3011));
	...
	public synchronized String connect(String userID) {
		...
		if (userID.startsWith("CON")) {
			...
			return "trueConnected to McGill Library Server.\nWelcome " + userID;
		} else {
			...
			writeLog(fileContent);
			return "falsCould not Connect to McGill Library Server.\nSorry " + userID;
		}
	}
	...
	public synchronized int borrowItem(String userID, String itemID) {
		if (library.containsKey(itemID)) {
			...
			if (contactOtherServer("CHEC" + userID, 3011).equals("0")) {
				writeLog("\nBorrow Request forwarded to Concordia Server");
				return Integer.parseInt(contactOtherServer("BORR" + userID + itemID, 3011));
			} else
				return 2;
	...
	public synchronized String findItem(String userID, String itemName) {
		...
		sb.append(contactOtherServer("FIND" + userID + itemName, 3011));
	...
	public synchronized boolean returnItem(String userID, String itemID) {
		if (borrowers.containsKey(userID)) {
			...
			return Boolean.getBoolean(contactOtherServer("RETU" + userID + itemID, 3011));
	...
	public synchronized boolean addToWaitingList(String userID, String itemID) {
		if (itemID.startsWith("CON")) {
			contactOtherServer("WAIT" + userID + itemID, 3011);
	...
	private void initiateUDPSocket() throws NotBoundException {
		DatagramSocket aSocket = null;
		try {
			aSocket = new DatagramSocket(3021);
			byte[] buffer = new byte[100000];
			System.out.println("McGill UDP Socket 3021 Initiated for inter library communication. MCGSS");
			while (true) {
				DatagramPacket request = new DatagramPacket(buffer, buffer.length);
				aSocket.receive(request);
				String requestMessage = new String(request.getData()).trim();
	...
	public int exchangeItem(String userID, String oldItemID, String newItemID) throws GeneralException {
		...
		if (oldItemID.startsWith(serverNickName)) {
			...
			if (contactOtherServer("OLDB" + userID + oldItemID, 3011).equals("true"))
				oldFlag = true;
		...
		if (newItemID.startsWith(serverNickName)) {
			...
			if (contactOtherServer("NEWB" + userID + newItemID, 3011).equals("true"))
				newFlag = true;
	...
	public static boolean serverHandler(boolean restartServer) throws Exception {
		...
		try (DatagramSocket aSocket = new DatagramSocket()) {
			String request = RepositoryOperationEnum.GET_LIBRARY_SERVER_DETAILS.name() + "#MCGCS";
		...
		mcgserver.updateDatabase(serverDetails.get(0), serverDetails.get(1));
