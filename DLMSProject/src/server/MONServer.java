...
/**
 * @author Yash Sheth MONCS port 3030 MONSS port 3031
 */
public class MONServer implements LibraryInterface {
	...
	private static int portCS = 3030;
	private static int portSS = 3031;
	...
	public static void main(String args[]) throws Exception {
		...
		try (DatagramSocket aSocket = new DatagramSocket();) {
			...
			System.out.println("Registering MONSS details with central repo.");
			...
			if (packet.getData().toString().equals("TRUE"))
				System.out.println("MONSS connection details are registered.");

			System.out.println("Registering MONCS details with central repo.");
			...
			aSocket.receive(packet);
			if (packet.getData().toString().equals("TRUE"))
				System.out.println("MONCS connection details are registered.");
		...
		System.out.println("Montreal server is up.");
	}
	...
	private void activateServer() throws AlreadyBoundException, IOException {
		...
		try {
			...
			System.out.println("Montreal UDP Socket 3030 Initiated for client library communication. MONCS");
			while (true) {
			...
			if (borrowers.containsKey(userID)) {
				...
				for (Book b : newBookList) {
					if (b.getID().equals(itemID)) {
						...
						writeLog(fileContent);
						fileContent = userID + " already has " + itemID+ " borrowed from the library. Check your Reading List. Happy Reading!";
						return 0;
					}
				}
			}
	...
	private void initiateUDPSocket() throws NotBoundException {
		DatagramSocket aSocket = null;
		try {
			aSocket = new DatagramSocket(3031);
			byte[] buffer = new byte[100000];
			System.out.println("Montreal UDP Socket 3031 Initiated for inter library communication. MONSS");
			while (true) {
				DatagramPacket request = new DatagramPacket(buffer, buffer.length);
				aSocket.receive(request);
				String requestMessage = new String(request.getData()).trim();
	...
	public static boolean serverHandler(boolean restartServer) throws Exception {
		...
		try (DatagramSocket aSocket = new DatagramSocket()) {
			String request = RepositoryOperationEnum.GET_LIBRARY_SERVER_DETAILS.name() + "#MONCS";
		...
		monserver.updateDatabase(serverDetails.get(0), serverDetails.get(1));
